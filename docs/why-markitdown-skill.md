...
这个 Skill 最初是为建设知识库而制作的。知识来源格式多样，可能是 PDF、Word、PowerPoint、Excel、HTML 或图片；在进入知识库前，需要先把这些资料转换为统一、适合检索和大语言模型处理的文本。
...
- 统一处理单个文件、多个文件和整个目录；
- 选择更安全的本地、流式或网络转换方式；
- 处理缺失依赖，并使用本地视觉模型读取独立图片和文档内嵌图片；
- 检查转换结果是否为空或明显缺失内容。

独立图片以及 PPTX、DOCX、PDF、XLSX 中的内嵌图片可通过 OpenAI 兼容视觉模型转换为文字。默认使用本机服务，也允许配置其他 URL；使用远程服务时，图片数据会发送到该服务。当前版本不支持音频和视频转录。
